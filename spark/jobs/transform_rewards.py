...
RAW_JSON_SCHEMA = StructType([
    ...
    StructField("total_daily_rate", DoubleType(),  True),
    StructField("rewards_min_size", DoubleType(),  True),
...
def build_opportunities(df: DataFrame) -> DataFrame:
    ...
    best_hour = (
        ...
        .agg(F.first(F.hour("fetched_at"), ignorenulls=True).alias("best_hour_utc"))
    )
    ...
    agg_24h = (
        ...
        .groupBy("condition_id")
        .agg(F.sum("score_per_maker").alias("simulated_rewards_24h"))
    )
